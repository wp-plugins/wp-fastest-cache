	...
	class WpFastestCache{
		...
		public function minifyCss($content){
			if(isset($this->options->wpFastestCacheMinifyCss)){
				preg_match_all("/href=[\"\'](.*?)\.css(.*?)[\"\']/", $content, $match, PREG_SET_ORDER);
				if(count($match) > 0){
					foreach ($match as $fileV) {
						$file = $fileV[1];
				    	$cssThemePath = strstr($file, 'wp-content');
				    	$cssThemePath = str_replace("wp-content", "", $cssThemePath);
					    $cachFilePath = $this->wpContentDir."/cache/all/".md5($cssThemePath);
					    if(!is_dir($cachFilePath)){
							if($cssContent = @file_get_contents($file.".css")){
								require_once "minify-css.php";
							    $themePath = substr($cssThemePath, 0, strripos($cssThemePath, "/"));
							    $cssContent = preg_replace('/url\((\"|\')?([^http\:\/\/\/]*)\//smi',"url($1../../..$themePath/$2/", $cssContent);

							    $cssContent = Minify_CSS_Compressor::process($cssContent, array());

							    $this->createFolder($cachFilePath, $cssContent, "css");
							}
					    }
					    $newFile = str_replace('wp-content', 'wp-content/cache/all/', $file);
					    $newFile = str_replace($cssThemePath, md5($cssThemePath), $newFile);
					    $content = str_replace($fileV[2] ? $file.".css".$fileV[2] : $file.".css", $newFile."/index.css?v=".time(), $content);
				  	}
				}
			}
			return $content;
