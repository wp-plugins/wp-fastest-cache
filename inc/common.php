	...
	class WpFastestCache{
		...
		public function optionsPage(){
			...
			<div class="wrap">
				...
				<? if($this->systemMessage){ ?>
					<div class="updated fade below-h2" id="message"><p><?=$this->systemMessage?></p></div>
				<? } ?>
				<div class="tabGroup">
					<?
						...
						foreach ($tabs as $key => $value){
							...
							}else if("wpfc-".$_POST["wpFastestCachePage"] == $value["id"]){
								$checked = ' checked="checked" ';
							}
				    ...
				    <div class="tab1">
						<form method="post" name="wp_manager">
							...
							<div class="questionCon">
								...
								<div class="inputCon"><input type="checkbox" <?=$wpFastestCacheStatus?> id="wpFastestCacheStatus" name="wpFastestCacheStatus"><label for="wpFastestCacheStatus"> Enable</label></div>
							</div>
							<div class="questionCon">
								<div class="question">New Post</div>
								<div class="inputCon"><input type="checkbox" <?=$wpFastestCacheNewPost?> id="wpFastestCacheNewPost" name="wpFastestCacheNewPost"><label for="wpFastestCacheNewPost"> Clear all cache files when a post or page is published</label></div>
							</div>
							<div class="questionCon">
				    ...
				    <div class="tab2">
				    	<form method="post" name="wp_manager">
				    		...
				    		<div class="questionCon">
				    			...
				    			Target folder <b><?=$this->wpContentDir?>/cache/all</b>
				    			</div>
				    		</div>
				    		<div class="questionCon qsubmit">
		...
		public function callback($buffer){
			...
			if(count($notCache) > 0){
				...
				return $buffer."<!-- WP Fastest Cache not cached version ".$this->wpContentDir." -->";
			}
		}
