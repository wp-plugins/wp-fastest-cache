		...
		public function optionsPage(){
			...
			<div class="wrap">
				...
				<div class="omni_admin_sidebar_section">
				  <h3>Having Issues?</h3>
				  <ul>
				    <li>You can create a ticket <a target="_blank" href="http://wordpress.org/support/plugin/wp-fastest-cache">WordPress support forum</a></li>
				  </ul>
				  </div>
				</div>
			</div>
					    ...
					    if(!is_dir($cachFilePath)){
					    	if($cachFilePath){
								if($cssContent = @file_get_contents($file.".css")){
								    ...
								    $content = str_replace($fileV[3] ? $file.".css".$fileV[3] : $file.".css", $newFile."/index.css?v=".time(), $content);
								}
					    	}
					    }else{
						    $newFile = str_replace('wp-content', 'wp-content/cache/all/', $file);
